...
local BlockPlaced = ReplicatedStorage:WaitForChild("BlockPlaced")
local Objects = ReplicatedStorage:WaitForChild("Objects")

--local mods = ServerStorage:WaitForChild("Modules")
local DataDeter = require(script.Parent.DataDeter2)
--local sec = require(mods.Secret)

DataDeter.SetServerSecret("HAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHAHA", 75) -- global security_level to 75, set the server secret

local plrData = DataDeter.InDataInfo("PlayersDataObject", "global", {
	security_level = 75,
...
-- block placed event
BlockPlaced.OnServerEvent:Connect(function(player, targetHitPos, targetHitInstance, targetHitNormal, rotationX, rotationY)
	...
	local index = tonumber(datas.obj_count) or 0	

	local block = blockUnclone:Clone()
	block.Parent = modelBase
	block.Name = block.Name .. tostring(index + 1)
	...
	local obj = {
		...
		color = { r = block.Color.R, g = block.Color.G, b = block.Color.B },
		size = { x = block.Size.X, y = block.Size.Y, z = block.Size.Z },
		pos = { x = block.Position.X, y = block.Position.Y, z = block.Position.Z },
		rightvec = { x = block.CFrame.RightVector.X, y = block.CFrame.RightVector.Y, z = block.CFrame.RightVector.Z },
		upvec = { x = block.CFrame.UpVector.X, y = block.CFrame.UpVector.Y, z = block.CFrame.UpVector.Z },
	}	
	...
	datas.previous_place_cframe = prevPlace
		
	local newIndex = (index + 1)
	
	datas.objects[newIndex] = obj
	datas.obj_count = newIndex
		
	print(datas.obj_count)
